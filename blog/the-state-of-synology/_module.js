...
function create_fragment$4(ctx) {
	...
	return {
		...
		h() {
			attr(div0, "class", "section-container content svelte-1qsoljh");
			attr(div1, "class", "section");
			attr(div2, "class", "section");
			attr(div2, "id", "section-ceba82b6");
		},
		m(target, anchor) {
			insert_hydration(target, div2, anchor);
...
function create_fragment$6(ctx) {
	...
	component_2 = new Component$3({
			props: {
				...
				title: "Pulumi > Terraform",
				description: "There can only be one IaC... not really, but really.",
				superhead: "JULY 23, 2023 ",
				heading: "The State of Synology"
			}
		});
	...
	component_3 = new Component$4({
			props: {
				...
				description: "There can only be one IaC... not really, but really.",
				content: {
					"html": "<p>I think it was around 2014 or 2015 when Synology got on my radar. Having never even seen the operating system yet, I was already in love with the form factor. This was before I had a 42U server rack in my house, so it seemed like the perfect little device for storing my data. But I was also fairly new to the workforce, and fairly new to being married, and trying to save up to buy a house, and a Synology (plus hard drives) was a bit out of reach. I took the long way around. I could talk all day about the several devices that came and went before 2018, but let's not go down that rabbit hole. A 42U server rack came and went along with several machines, and then I finally made my dream come true. I started off with a DS918+, and I couldn't have been happier. I had helped my friend Nate with his Synology several times at that point (you're welcome, Nate), and I knew it was what I wanted. So I bought the unit, threw in 4x 2TB WD Reds (CMR), and I was off to the races. It was great. It handled DDNS, it was a VPN server, it had Docker, and it had Let's Encrypt. That's totally ignoring their amazing suite of apps including Synology Drive, Synology Office, DS Video, DS Photo (now Synology Photos), and a number of others. The people that developed DSM had thought of it all, and I was so happy with the ease of use. I was so happy that I even upgraded to the DS1821+.</p><p>But…</p><p>We both grew up, and we started to grow apart. Even before I upgraded to the DS1821+, there was growing tension between DSM and I. I wanted more out of my Synology, and it was frustrating because it was Synology that was stopping me from doing more. It started with the SSL certificates. If you want to use a wildcard certificate, you could only do so with Synology's pre-loaded domain, not your custom domain. I still don't understand why they would implement a limitation like that, but it forced me to find a work around. Eventually, I settled on the <a target=\"_blank\" rel=\"noopener noreferrer nofollow\" class=\"link link\" href=\"https://fleet.linuxserver.io/image?name=linuxserver/swag\">SWAG Docker container from LinuxServer.io</a>. This solved my problem with my SSL certificate, but it also forced me to do two things I didn't really want to do: ditch Synology's built-in reverse proxy, and write a ton of little Nginx config files. The built-in reverse proxy doesn't support any advanced configuration, but it was doing the trick, and SWAG container was more work than I was hoping to have to put in. Fortunately, I later learned about <a target=\"_blank\" rel=\"noopener noreferrer nofollow\" class=\"link link\" href=\"https://nginxproxymanager.com/\">NGINX Proxy Manager</a>, but as you probably expected, that wasn't without its issues either.</p><p>NGINX Proxy Manager (NPM) is a dream come true as far as setting up a reverse proxy and an SSL certificate. It couldn't be easier… except that Synology reserves port 80 and port 443 for its own uses. I could have used different ports, and then re-routed all port 80 and 443 requests on my router to the different ports on the Synology, but I didn't want to, nor should I have had to. So after a brief tinkering, I moved Synology's reserved ports from 80 and 443 to 81 and 444, respectively, and NPM was working like a dream. This breaks every time I update my Synology though, so maybe I should have used different ports.</p><p>Perhaps my biggest frustration was with Docker. I haven't used Synology's on-board Docker UI in years because it's broken. By \"broken\", I mean that I am using features that are not supported by it. Instead of it just ignoring those features, it prevents the UI from working correctly. And should I make a change in the UI, it will fail to maintain the unsupported configuration, thus breaking my containers. Quite simply, if you want to do anything other than use a bridge or host network, you will break the UI. So when I wanted to use one container as a VPN gateway for another container, it broke the UI. When I wanted to use a Macvlan network to give a container its own local IP address, it broke the UI. I was using the CLI for most of this anyway, but it would have been nice to be able to use the UI to monitor my containers. I have been using <a target=\"_blank\" rel=\"noopener noreferrer nofollow\" class=\"link link\" href=\"https://www.portainer.io/\">Portainer</a> for the past few years, and it has been great. It even solves the other major issue that Synology forced on its users regarding Docker: the API. Synology crippled the power user experience of Docker by locking down the API and making it inaccessible. Portainer has its own API, so if you use it to manage your containers, then you have a pretty easy work around.</p><p>I mentioned previously how I wanted to use a container as a VPN gateway for another container. This was easier said than done. As expected, this docker configuration broke the Docker UI, but it also just didn't work. The reason it didn't work was because the TUN kernel module wasn't loaded. I had to set up a scheduled task to load the TUN kernel module on boot. And yes, it has to run on boot, because every time you restart your Synology, that configuration does not persist.</p><p>I feel like I have illustrated my point. DSM7.2 came out recently, and it has a new Docker UI called \"Container Manager\". I will never know if it works though, because I use containers as network devices, and I use Macvlan networks, and I don't want to end up having to fix all of my configuration just because I wanted to see if Synology stopped nerfing the Docker experience.</p><p>I know that these things I'm trying to do aren't supported. I know that I am pushing the limits of what the device is capable of. What I don't understand is why Synology would deliberately get in my way and prevent me from doing these things though. The first time you SSH into your Synology, you get a warning. Other than that, it lets you do what you want. Why can't I just have a warning. Just warn me that I exposed my Docker API, or warn me that Macvlan networks are unsupported. I'm not suggesting that Synology make changes to their platform to accommodate these extended use cases, but I think they should stay out of the way. Even MacOS doesn't get in the way when I want to do things it doesn't like.</p><p>Ultimately, the DS1821+ will be my last Synology. They are continuing to tighten things up, and are working toward vendor lock-in, and it's not sitting well with me. A number of the features and functions I wanted to use the most in DSM are seemingly only half-baked, and they were not robust enough solutions to prevent me from needing to use 3rd party applications. I cannot imagine upgrading to a more expensive, more powerful device only to have these same problems, and then also being forced into buying their brand-name, prohibitively expensive hard drives as well. Synology seems to be entering this weird place where they are pricing themselves out of home labs, and nerfing their features so they could never be used in an enterprise environment. Maybe small business is where the money's at?</p>",
					"markdown": "I think it was around 2014 or 2015 when Synology got on my radar. Having never even seen the operating system yet, I was already in love with the form factor. This was before I had a 42U server rack in my house, so it seemed like the perfect little device for storing my data. But I was also fairly new to the workforce, and fairly new to being married, and trying to save up to buy a house, and a Synology (plus hard drives) was a bit out of reach. I took the long way around. I could talk all day about the several devices that came and went before 2018, but let's not go down that rabbit hole. A 42U server rack came and went along with several machines, and then I finally made my dream come true. I started off with a DS918+, and I couldn't have been happier. I had helped my friend Nate with his Synology several times at that point (you're welcome, Nate), and I knew it was what I wanted. So I bought the unit, threw in 4x 2TB WD Reds (CMR), and I was off to the races. It was great. It handled DDNS, it was a VPN server, it had Docker, and it had Let's Encrypt. That's totally ignoring their amazing suite of apps including Synology Drive, Synology Office, DS Video, DS Photo (now Synology Photos), and a number of others. The people that developed DSM had thought of it all, and I was so happy with the ease of use. I was so happy that I even upgraded to the DS1821+.\n\nBut…\n\nWe both grew up, and we started to grow apart. Even before I upgraded to the DS1821+, there was growing tension between DSM and I. I wanted more out of my Synology, and it was frustrating because it was Synology that was stopping me from doing more. It started with the SSL certificates. If you want to use a wildcard certificate, you could only do so with Synology's pre-loaded domain, not your custom domain. I still don't understand why they would implement a limitation like that, but it forced me to find a work around. Eventually, I settled on the [SWAG Docker container from LinuxServer.io](<https://fleet.linuxserver.io/image?name=linuxserver/swag>). This solved my problem with my SSL certificate, but it also forced me to do two things I didn't really want to do: ditch Synology's built-in reverse proxy, and write a ton of little Nginx config files. The built-in reverse proxy doesn't support any advanced configuration, but it was doing the trick, and SWAG container was more work than I was hoping to have to put in. Fortunately, I later learned about [NGINX Proxy Manager](<https://nginxproxymanager.com/>), but as you probably expected, that wasn't without its issues either.\n\nNGINX Proxy Manager (NPM) is a dream come true as far as setting up a reverse proxy and an SSL certificate. It couldn't be easier… except that Synology reserves port 80 and port 443 for its own uses. I could have used different ports, and then re-routed all port 80 and 443 requests on my router to the different ports on the Synology, but I didn't want to, nor should I have had to. So after a brief tinkering, I moved Synology's reserved ports from 80 and 443 to 81 and 444, respectively, and NPM was working like a dream. This breaks every time I update my Synology though, so maybe I should have used different ports.\n\nPerhaps my biggest frustration was with Docker. I haven't used Synology's on-board Docker UI in years because it's broken. By \"broken\", I mean that I am using features that are not supported by it. Instead of it just ignoring those features, it prevents the UI from working correctly. And should I make a change in the UI, it will fail to maintain the unsupported configuration, thus breaking my containers. Quite simply, if you want to do anything other than use a bridge or host network, you will break the UI. So when I wanted to use one container as a VPN gateway for another container, it broke the UI. When I wanted to use a Macvlan network to give a container its own local IP address, it broke the UI. I was using the CLI for most of this anyway, but it would have been nice to be able to use the UI to monitor my containers. I have been using [Portainer](<https://www.portainer.io/>) for the past few years, and it has been great. It even solves the other major issue that Synology forced on its users regarding Docker: the API. Synology crippled the power user experience of Docker by locking down the API and making it inaccessible. Portainer has its own API, so if you use it to manage your containers, then you have a pretty easy work around.\n\nI mentioned previously how I wanted to use a container as a VPN gateway for another container. This was easier said than done. As expected, this docker configuration broke the Docker UI, but it also just didn't work. The reason it didn't work was because the TUN kernel module wasn't loaded. I had to set up a scheduled task to load the TUN kernel module on boot. And yes, it has to run on boot, because every time you restart your Synology, that configuration does not persist.\n\nI feel like I have illustrated my point. DSM7.2 came out recently, and it has a new Docker UI called \"Container Manager\". I will never know if it works though, because I use containers as network devices, and I use Macvlan networks, and I don't want to end up having to fix all of my configuration just because I wanted to see if Synology stopped nerfing the Docker experience.\n\nI know that these things I'm trying to do aren't supported. I know that I am pushing the limits of what the device is capable of. What I don't understand is why Synology would deliberately get in my way and prevent me from doing these things though. The first time you SSH into your Synology, you get a warning. Other than that, it lets you do what you want. Why can't I just have a warning. Just warn me that I exposed my Docker API, or warn me that Macvlan networks are unsupported. I'm not suggesting that Synology make changes to their platform to accommodate these extended use cases, but I think they should stay out of the way. Even MacOS doesn't get in the way when I want to do things it doesn't like.\n\nUltimately, the DS1821+ will be my last Synology. They are continuing to tighten things up, and are working toward vendor lock-in, and it's not sitting well with me. A number of the features and functions I wanted to use the most in DSM are seemingly only half-baked, and they were not robust enough solutions to prevent me from needing to use 3rd party applications. I cannot imagine upgrading to a more expensive, more powerful device only to have these same problems, and then also being forced into buying their brand-name, prohibitively expensive hard drives as well. Synology seems to be entering this weird place where they are pricing themselves out of home labs, and nerfing their features so they could never be used in an enterprise environment. Maybe small business is where the money's at?\n\n"
				}
			}
		});
